...
def create_submitted_applicant(
    ...
    applicant = Applicant(
        ...
        applied_from=data.personal_details.applied_from,
        source_name=data.personal_details.source_name,
...
def list_candidates_advanced(
    ...
    statuses_filter: Optional[str] = None,
    ...
    # 1. Filters
    if statuses_filter:
        # Comma-separated list of statuses (e.g. "TECH_ROUND_1,TECH_ROUND_2")
        status_list = [s.strip() for s in statuses_filter.split(",") if s.strip()]
        if status_list:
            query = query.filter(func.lower(Applicant.status).in_([s.lower() for s in status_list]))
    elif status_filter:
...
def update_applicant(
    ...
    if data.personal_details:
        ...
        applicant.applied_from = pd.applied_from
        applicant.source_name = pd.source_name
