...
class FinalDecisionRequest(BaseModel):
    final_status: str = Field(..., max_length=20)  # SELECTED, REJECTED, HOLD
    ...
    def validate_final_status(cls, v):
        upper = v.upper()
        if upper not in ("SELECTED", "REJECTED", "HOLD"):
            raise ValueError("Status must be SELECTED, REJECTED, or HOLD")
        return upper

    @model_validator(mode="after")
    def validate_ctc_requirement(self):
        if self.final_status == "SELECTED" and (self.offered_ctc is None or self.offered_ctc <= 0):
