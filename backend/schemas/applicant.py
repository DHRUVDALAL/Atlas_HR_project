...
VALID_GENDERS = {"MALE", "FEMALE", "OTHER", "OTHERS"}
VALID_EMPLOYMENT_TYPES = {"FULL_TIME", "PART_TIME", "CONTRACT", "INTERN"}
VALID_OPTIONS = {"A", "B", "C", "D"}
VALID_APPLIED_FROM = {"LINKEDIN", "NAUKRI", "VENDOR", "REFERRAL"}
VALID_STATUSES = {
    "DRAFT", "SUBMITTED", "Submitted — awaiting reception", "RECEPTION_FORWARDED",
    "TECH_ROUND_1", "TECH_ROUND_2", "TECH_ROUND_3", "TECH_ROUND_4", "TECH_ROUND_5",
    "CEO_ROUND", "FINAL_DISCUSSION_PENDING", "FINAL_DECISION",
...
class ApplicantCreate(BaseModel):
    ...
    # Form Header fields
    position_applied_for: Optional[str] = Field(None, max_length=100)
    referred_by: Optional[str] = Field(None, max_length=100)
    reference_number: Optional[str] = Field(None, max_length=50)
    applied_from: Optional[str] = Field(None, max_length=20)
    ...
    @field_validator("applied_from")
    @classmethod
    def validate_applied_from(cls, v):
        if v is None:
            return v
        upper = v.upper()
        if upper not in VALID_APPLIED_FROM:
            raise ValueError(f"applied_from must be one of {VALID_APPLIED_FROM}")
        return upper
...
class DocumentResponse(BaseModel):
    ...
    document_type: str
    file_name: str
    # file_path is intentionally omitted — the server-side absolute path must
