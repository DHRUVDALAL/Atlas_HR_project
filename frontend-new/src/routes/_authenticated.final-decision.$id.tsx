...
export const Route = createFileRoute("/_authenticated/final-decision/$id")({
  ...
  return (
    ...
    >
      ...
      <div className="grid lg:grid-cols-5 gap-8 items-start">
        {/* Left: Read-only panel feedback */}
        <div className="lg:col-span-3 space-y-6">
          <h2 className="text-xl font-bold tracking-tight text-[#1d1d1f]">Panel Feedback</h2>
                ...
                {techRounds.map((r) => (
                  <div key={r.round_id} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                    <div className="flex items-center justify-between mb-4">
                      <div className="text-lg font-bold tracking-tight text-[#1d1d1f]">Technical Round #{r.round_number}</div>
                    ...
                    {r.remarks && (
                      <div className="bg-[#fbfbfd] p-4 rounded-2xl border border-gray-50">
                        ...
                        <p className="text-sm font-medium text-[#1d1d1f] leading-relaxed">{r.remarks}</p>
                      </div>
                    )}
                  </div>
                ))}
            ...
            {activeTab === "ceo" && ceoRounds.length > 0 && (
              ...
              >
                {ceoRounds.map((r) => (
                  <div key={r.round_id} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
                    <div className="flex items-center justify-between mb-4">
                      <div className="text-lg font-bold tracking-tight text-[#1d1d1f]">CEO Evaluation</div>
                      <span className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider ${
                        r.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-700'
                      }`}>
                        {r.status}
                    ...
                    {r.remarks && (
                      <div className="bg-[#fbfbfd] p-4 rounded-2xl border border-gray-50">
                        <div className="text-[10px] font-bold text-[#86868b] uppercase tracking-widest mb-2">Executive Remarks</div>
                        <p className="text-sm font-medium text-[#1d1d1f] leading-relaxed">{r.remarks}</p>
                      </div>
                    )}
                  </div>
        ...
        <div className="lg:col-span-2 space-y-6 lg:sticky lg:top-8">
          <h2 className="text-xl font-bold tracking-tight text-[#1d1d1f]">Decision Formulation</h2>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] space-y-6">
            ...
            <div className="border-t border-gray-100 pt-6 space-y-6">
              <div className="space-y-3">
                <label className="text-[11px] font-bold text-[#86868b] uppercase tracking-wider ml-1">Final Outcome</label>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              ...
              >
                <Send className="h-4 w-4 mr-2" /> Confirm Decision
              </button>
            </div>
          </div>
        </div>
