...
export const Route = createFileRoute("/_authenticated/hr/queue")({
  head: () => ({ meta: [{ title: "HR Review Queue — Atlas HR" }] }),
  component: HrReviewQueue,
});

function HrReviewQueue() {
  const [status, setStatus] = useState<string>("RECEPTION_FORWARDED");
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["applicants", "hr-queue", status, debouncedSearch, page, sortBy, sortOrder],
    queryFn: () =>
      api<{
        success: boolean;
        total: number;
  ...
  // Fetch all candidates for KPI counts
  ...
  return (
    ...
    >
      {/* Header */}
      <header>
        <h1 className="text-3xl font-bold tracking-tighter text-[#1d1d1f]">HR Review Queue</h1>
        <p className="text-sm font-medium text-[#86868b] mt-1">
          Manage candidate screenings and route profiles to technical assessment pipelines.
        </p>
      </header>
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-shadow duration-300">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-4xl font-bold tracking-tighter text-[#1d1d1f] mt-4">{countAwaitingHr}</div>
              </div>
              <div className="h-10 w-10 rounded-2xl bg-violet-50 grid place-items-center">
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-shadow duration-300">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-4xl font-bold tracking-tighter text-[#1d1d1f] mt-4">{countHrCompleted}</div>
              </div>
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-shadow duration-300">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-4xl font-bold tracking-tighter text-[#1d1d1f] mt-4">{countTechnical}</div>
              </div>
              <div className="h-10 w-10 rounded-2xl bg-purple-50 grid place-items-center">
          ...
          <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col justify-between hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-shadow duration-300">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-4xl font-bold tracking-tighter text-[#1d1d1f] mt-4">{countSelected}</div>
              </div>
      ...
      <div className="bg-white p-4 md:p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] space-y-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <SelectTrigger className="w-[200px] bg-[#f5f5f7] border-transparent rounded-2xl h-11 px-4 text-sm font-medium">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="rounded-2xl border-gray-100 shadow-xl">
                <SelectItem value="ALL" className="rounded-xl cursor-pointer">All statuses</SelectItem>
                <SelectItem value="RECEPTION_FORWARDED" className="rounded-xl cursor-pointer">Awaiting HR Review</SelectItem>
                <SelectItem value="TECHNICAL_PENDING" className="rounded-xl cursor-pointer">Technical Round 1</SelectItem>
                <SelectItem value="CEO_ROUND" className="rounded-xl cursor-pointer">CEO Round</SelectItem>
                <SelectItem value="FINAL_DISCUSSION_PENDING" className="rounded-xl cursor-pointer">Final Discussion</SelectItem>
                <SelectItem value="SELECTED" className="rounded-xl cursor-pointer">Selected</SelectItem>
                <SelectItem value="REJECTED" className="rounded-xl cursor-pointer">Rejected</SelectItem>
                <SelectItem value="ON_HOLD" className="rounded-xl cursor-pointer">On Hold</SelectItem>
              </SelectContent>
            </Select>
            <Select
              value={sortBy}
            ...
            {(status !== "RECEPTION_FORWARDED" || debouncedSearch) && (
              <button
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        ...
        <div className="grid grid-cols-[2fr_1.5fr_2fr_1fr_1.5fr_auto] gap-4 bg-[#fbfbfd] px-8 py-5 border-b border-gray-100 items-center">
          ...
          <div className="text-[11px] font-bold uppercase tracking-widest text-[#86868b] text-right w-24">Actions</div>
        </div>
