...
export const Route = createFileRoute("/_authenticated/offer/queue")({
  head: () => ({
    meta: [{ title: "Offer Queue — Atlas HR" }],
  }),
  component: OfferQueue,
});

type OfferFilter = "all" | "pending" | "sent" | "accepted" | "declined";

const statusParamMap: Record<OfferFilter, string | undefined> = {
  all: undefined,
  pending: "DRAFT",
  sent: "SENT",
  accepted: "ACCEPTED",
...
function OfferQueue() {
  ...
  const { data: offersData, isLoading } = useQuery<OffersResponse>({
    ...
    queryFn: () => api<OffersResponse>(`/api/offers?${params.toString()}`),
  });
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tighter text-[#1d1d1f]">Offer Queue</h1>
          <p className="text-sm font-medium text-[#86868b] mt-1">
            Manage offers for selected candidates.
          </p>
        </div>
        <Link 
          to="/offer/dashboard"
        ...
        >
          <FileCheck className="h-4 w-4 mr-2" /> Dashboard
        </Link>
      </header>
      ...
      <div className="bg-white p-4 md:p-6 rounded-[2rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] space-y-4">
        <div className="flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            {(["all", "pending", "sent", "accepted", "declined"] as OfferFilter[]).map((f) => (
              <button
                key={f}
                onClick={() => {
                  setFilter(f);
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        {isLoading ? (
          ...
          <div className="p-16 text-center">
            <FileCheck className="h-12 w-12 text-gray-200 mx-auto mb-4" />
            ...
            <p className="text-sm font-medium text-[#86868b] mt-1">
              {search
                ...
                : filter !== "all"
                  ...
                  : "No selected candidates yet. Offers will appear here once candidates reach SELECTED status."}
            </p>
          </div>
        ) : (
          <div className="flex flex-col">
            {items.map((item) => {
              ...
              const isPending = !item.status || statusUpper === "DRAFT";
              
              return (
                <div key={item.offer_id ?? item.candidate_id} className="grid grid-cols-[auto_1fr_auto_auto] gap-6 px-6 md:px-8 py-5 border-b border-gray-50/50 hover:bg-[#f5f5f7]/60 transition-colors items-center">
                  ...
                  <div className="min-w-0 pr-4">
                    ...
                    <div className="text-xs font-medium text-[#86868b] truncate mt-0.5">
                      {item.application_number} · {item.candidate_email}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end w-28">
                    <Link
                      ...
                      className={`flex items-center ${isPending ? "bg-[#0066cc] hover:bg-[#005bb5] text-white" : "bg-transparent hover:bg-gray-100 border border-gray-200 text-[#1d1d1f]"} rounded-full px-5 py-2 text-xs font-bold transition-all shadow-sm active:scale-[0.98]`}
                    >
                      {isPending ? (
                        <>Build <ArrowRight className="h-3.5 w-3.5 ml-1.5" /></>
                      ) : (
